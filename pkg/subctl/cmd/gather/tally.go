...
<table>
  <tr>
    ...
    <td>{{.Versions.SubctlVersion}}</td>
  </tr>
  <tr>
    <td>Submariner version:</td>
    <td>{{.Versions.SubmVersion}}</td>
  </tr>
  <tr>
    <td>Kubernetes Server version:</td>
    <td>{{.Versions.K8sServerVersion}}</td>
  </tr>
</table>
...
func getVersions(info Info) version {
	...
	Versions := version{
		SubctlVersion:    subctlversion.Version,
		SubmVersion:      submVer,
		K8sServerVersion: k8sServerVersion.String(),
	}
	return Versions
}
...
func WriteToHTML(fileWriter io.Writer, cData data) {
	t := template.Must(template.New("layout.html").Parse(tmpl))
